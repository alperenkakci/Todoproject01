...
type TodoRepository interface {
	...
	AddTodoItem(listID string, item TodoMessage) (Todo, error)
}
...
func (s *Service) AddTodoItem(listID string, item TodoMessage) (Todo, error) {
	item.CreationDate = time.Now()
	item.ModificationDate = time.Now()
	return s.repo.AddTodoItem(listID, item)
}
